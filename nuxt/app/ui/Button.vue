...
const props = withDefaults(defineProps<{ variant?: string, text: string, link?: string }>(), { variant: 'solid', link: '#' })
...
<template>
  <a :href="props.link" :class="['inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors disabled:pointer-events-none disabled:opacity-50 focus-visible:rounded-lg focus-visible:border-transparent focus-visible:no-underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-500 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0', variants[props.variant]]">{{ props.text }}</a>
</template>
