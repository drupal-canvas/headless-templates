...
- `src/server.ts`: static assets, generated manifest, and the adapter's `createCanvasHandler`. Configure actual deployment hostnames in `angular.json` → `security.allowedHosts`; do not allow arbitrary forwarded hosts.
